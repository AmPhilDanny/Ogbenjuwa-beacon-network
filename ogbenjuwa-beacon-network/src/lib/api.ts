const API_BASE = import.meta.env.VITE_API_BASE || 'https://ogbenjuwa-backend.vercel.app/api/v1';
...
// Dynamic import to avoid circular deps — only loaded when offline queue used
...
class ApiClient {
  ...
  clearTokens(): void {
    ...
    sessionStorage.removeItem('ogbenjuwaAuth');
  }
  ...
  private async request<T>(
    ...
    try {
      ...
      // If it's a network error and we're offline, queue mutating requests
      if (e instanceof TypeError && method !== 'GET' && !navigator.onLine) {
        ...
        throw new ApiClientError(
          'Request queued — will sync when back online',
          'OFFLINE_QUEUED',
          0,
        );
      }
      // Rethrow API client errors as-is
      if (e instanceof ApiClientError) throw e;
      // Wrap network errors for non-mutating requests
      throw new ApiClientError(
