...
import { Shield, Phone, KeyRound, ChevronRight, ChevronLeft, AlertCircle, Clock } from 'lucide-react';
  ...
  const handleOTPComplete = useCallback(async (value: string) => {
    ...
    try {
      ...
      if (newAttempts >= 3) {
        ...
        toast.error(`Wrong OTP. Attempt ${newAttempts} of 3.`);
      }
    } finally {
      setSubmitting(false);
    }
  }, [attempts, lockedUntil, phone, login, navigate, from]);
        ...
        <Card className="border-ogbenjuwa-green-mid/20 bg-ogbenjuwa-ink-mid/50">
          <CardHeader>
            ...
            <CardDescription className="text-center text-muted-foreground">
              ...
              {step === 'otp' && `Verification code sent to ${phone}`}
            </CardDescription>
          </CardHeader>
            ...
            {step === 'otp' && (
              <>
                ...
                <Button
                  ...
                  disabled={submitting}
                >
                  <ChevronLeft className="mr-1 h-4 w-4" /> Back to phone entry
                </Button>
              </>
            )}
          </CardContent>
